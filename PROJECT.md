...
- **Alquiler de habitaciones de Galileu 342**: entra siempre en la cuenta BBVA **Conjunta** (titular `Conjunta`). Dos conceptos con tratamiento distinto:
  - **Renta de habitación** (Tipo `income`, Categoría `Alquiler habitaciones`): cuenta en el cash flow y como ingreso NO salarial del hogar (aparece en el Dashboard general vía `get_monthly_summary`). NO es renta de trabajo (`is_renta_trabajo` y `is_nomina` devuelven False para "Alquiler habitación N", verificado 2026-09-01). Habitación 2 = 650 €/mes, pagada en **efectivo**, primera renta registrada el 01/09/2026 con Banco `Efectivo` (misma convención que el resto de movimientos en metálico de la hoja). Al caer el 01/09 no afecta al cash flow de agosto. Habitación 1 = renta pendiente de definir.
